...
Lib_lgr = "libcloudphxx_lgr_pytest"
...
def condensation(lib, lib_type, press = None, T = None,
    ...
    elif lib_type == 3:#TODO lagrangowski
        rv0 = rv.copy()
        print "rv0 przed", rv0
...
def cond_all(libname, libname_2m, libname_wrf, libname_2mor, libname_lgr, rc_0, sup_lim, sup_step, 
             temp_0, press_0):
    rc_an_l = []
    rc_xx_l = []
    rc_2xx_l = []
    ...
    for rv_0 in np.arange(rsat_an, rsat_an + sup_lim + sup_step, sup_step):
        ...
        rv_lgr, rc_lgr = condensation(lib=libname_lgr, lib_type=3, T=np.array(temp_0),
                                      press = np.array(press_0),
                                      rv=np.array(rv_0), rc=np.array(rc_0))
        ...
        rc_lgr_l.append(rc_lgr - rc_0)
        r_rsat_an_l.append(r_rsat_an)

    
    print "rc_xx po", rc_xx_l,"\n", "rc_2xx po", rc_2xx_l,"\n","rc_wrf", rc_wrf_l, "\n", "rc_2mor", rc_2mor_l, "\n", "rc_lgr", rc_lgr_l, "\n", "rc_an", rc_an_l
    return r_rsat_an_l, rc_xx_l, rc_2xx_l, rc_wrf_l, rc_2mor_l, rc_lgr_l, rc_an_l 


def figure_plot(ax, temp, libxx_1m, libxx_2m, libwrf_kes, lib_2mor, lib_lgr,
                rc_0, sup_lim, sup_step, label_y):    
    r_rsat, c_xx, c_2xx, c_wrf, c_2mor, c_lgr, c_an = cond_all(libxx_1m, libxx_2m, 
             libwrf_kes, lib_2mor, lib_lgr, rc_0, sup_lim, sup_step,
             temp_0 = temp, press_0 = 900.e2)

    plt.plot(r_rsat, c_an, "k--", r_rsat, c_xx, "r", r_rsat, c_2xx, "r .", 
             r_rsat, c_wrf, "g", r_rsat, c_2mor, "g.", r_rsat, c_lgr, "b*",
             r_rsat, len(r_rsat)*[0], 'k--', len(c_an)*[0], c_an, 'k--', markersize=10 )
    plt.legend(["analytic","libcloudphxx_1m", "libcloudphxx_2m", 
                "wrf_kessler", "wrf_morrison", "libcloudph_lgr"],
    ...
    ax.set_xlim(0,  sup_lim)
    ax.set_ylim(0, max(c_an))
...
def figure_plot_all(temp_list):
    ...
    for nr, temp in enumerate(temp_list):
        ...
        figure_plot(ax, temp, libxx_1m=Libxx_1m, libxx_2m=Libxx_2m, 
                    libwrf_kes=Libwrf_kes, lib_2mor=Lib_2mor, lib_lgr=Lib_lgr,
                    rc_0=.1e-4, sup_lim=1.e-3, sup_step=0.1e-3, 
                    label_y=label_y)
